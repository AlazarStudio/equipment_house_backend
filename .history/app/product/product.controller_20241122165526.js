...
export const createNewProduct = asyncHandler(async (req, res) => {
  ...
  // Проверяем, существует ли businessSolution, если указан
  if (businessSolutionId) {
    const businessSolution = await prisma.businessSolution.findUnique({
      where: { id: businessSolutionId },
    });

    if (!businessSolution) {
      res.status(400);
      throw new Error('Invalid businessSolutionId');
    }
  }
