  ...
  try {
    ...
    export const updateOrder = asyncHandler(async (req, res) => {
      try {
        ...
        // Удаление товаров, которые больше не в заказе
        const orderItemIds = items.map((item) => item.productId);
        const deletedItems = await prisma.orderItem.deleteMany({
          where: {
            orderId: parseInt(orderId),
            productId: { notIn: orderItemIds }, // Удаляем товары, которых нет в новом заказе
          },
        });

        // Обновление или добавление товаров
