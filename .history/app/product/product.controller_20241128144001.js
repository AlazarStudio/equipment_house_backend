...
export const createNewProduct = [
  upload.single('img'), // Middleware для обработки изображения
  asyncHandler(async (req, res) => {
    ...
    const imgPath = `/uploads/products/${req.file.filename}`; // Путь к изображению
    ...
    if (!category) {
      res.status(400).json({ message: 'Категория с таким ID не найдена!' });
      return;
    }
    ...
    const product = await prisma.product.create({
      data: {
        ...
        img: imgPath,
        type,
        availability: availability === 'true', // Преобразуем строку в boolean
        code,
        description,
        characteristics,
