...
export const createNewProduct = asyncHandler(async (req, res) => {
  ...
  const images = req.files
    ? req.files.map((file) => `/uploads/${file.filename.replace('./', '')}`)
    : [];

  console.log('Пути изображений для сохранения:', images);
  ...
  const product = await prisma.product.create({
    data: {
      name,
      price,
      img: images, // Сохраняем изображения
      type,
      availability,
      code,
      description,
  ...
  console.log('Сохранённый продукт:', product);
