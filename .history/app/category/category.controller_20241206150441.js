...
export const updateCategory = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { title, img, imagesRaw } = req.body;

  // Проверяем, если img или imagesRaw не пустые, объединяем их
  const images = [
    ...(img || []), // Старая информация об изображениях
    ...(imagesRaw || []).map((image) =>
      typeof image === 'object' ? `/uploads/${image.rawFile.path}` : image
    ),
  ];
  ...
  if (!title || !images.length) {
    res.status(400).json({ error: 'Title and images are required for update' });
    return;
  }
  ...
  try {
    const updatedCategory = await prisma.category.update({
      where: { id: parseInt(id, 10) },
      data: { title, img: images }, // Сохраняем объединенные изображения
    });
