...
// Настройка `multer` для загрузки файлов (для XML и изображений)
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowedTypes = /xml|image/; // Разрешаем только XML и изображения
    ...
    cb(new Error('Ошибка: допустимы только XML файлы и изображения'));
  },
});
...
// Маршрут для загрузки XML и изображений
app.post('/api/upload-xml', upload.fields([{ name: 'file', maxCount: 1 }, { name: 'image', maxCount: 5 }]), async (req, res) => {
  try {
    if (!req.files['file']) {
      return res.status(400).json({ message: 'XML файл не загружен' });
    }
    ...
    await saveDataToDatabase(shopData, req.files['image']);

    res.status(200).json({ message: 'XML и изображения успешно загружены и обработаны', data: shopData });
  } catch (error) {
    console.error('Ошибка обработки XML:', error);
    res.status(500).json({ message: 'Ошибка обработки XML и изображений', error: error.message });
  }
});

// Функция сохранения данных в базу Prisma
const saveDataToDatabase = async (shop, images) => {
  ...
  if (images && images.length > 0) {
    const imagePaths = images.map(image => `/uploads/images/${image.filename}`);
    for (const imagePath of imagePaths) {
      await prisma.productImage.create({
        data: {
          imageUrl: imagePath, // Сохраняем путь к изображению
        },
      });
    }
  }
