...
export const createNewCategory = asyncHandler(async (req, res) => {
  ...
  const category = await prisma.category.create({
    data: {
      title,
      img: images, // img теперь одиночная строка
    },
  });
...
export const updateCategory = asyncHandler(async (req, res) => {
  ...
  // Если img - это одиночное изображение, то не используем map
  let images = [];
  if (img) {
    if (Array.isArray(img)) {
      images = img.map((image) => (typeof image === 'object' ? `/uploads/${image.rawFile.path}` : image));
    } else {
      images.push(`/uploads/${img.rawFile.path}`); // Для одиночного изображения
    }
  }
  ...
  try {
    const updatedCategory = await prisma.category.update({
      where: { id: parseInt(id, 10) }, // Убедитесь, что id парсится как целое число
      data: { title, img: images },
    });
