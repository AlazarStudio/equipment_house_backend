    ...
    try {
      ...
      if (!fs.existsSync(uploadDir)) {
        try {
          ...
          console.log('Папка uploads успешно создана:', uploadDir);
        } catch (err) {
          console.error('Ошибка при создании папки uploads:', err);
          return res.status(500).json({ message: 'Ошибка при создании папки' });
        }
      }
      ...
      for (const file of files) {
        ...
        try {
          ...
          console.error(`Ошибка при сохранении файла ${fileName}:`, writeError);
          return res
            .status(500)
            .json({ message: `Ошибка при сохранении файла: ${fileName}` });
        }
      }

      console.log('Загруженные файлы сохранены:', filePaths);
